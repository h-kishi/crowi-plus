module.exports = function(crowi, app) {
  ...
  /**
   * return a Promise instance for retrieving User
   * @param {string} id
   */
  var getPromiseForRetrievingUser = function(id) {
    let promise = undefined;

    if (id.includes('@')) {
      debug(`logging in with email`);
      promise = User.findUserByEmail(id);
    }
    else {
      debug(`logging in with username`);
      promise = User.findUserByUsername(id);
    }
  ...
  actions.login = function(req, res) {
    ...
    const promise = getPromiseForRetrievingUser(id);
    promise.then((userData) => {
      ...
      // verify password
      if (!userData.isPasswordValid(password)) {
